...
#include <string.h>
#include <time.h>
/* ---- end autoload platform abstractions ---- */
 ...
 *
 * When a ROM is loaded, this feature reads a config file next to the
 * core library ("swanstation_libretro.dll" / "swanstation_libretro.so" /
 * "swanstation_libretro.dylib") and, if enabled, automatically loads
 * a save state for that ROM.
 *
 * Windows : uses GetModuleHandleExA / GetModuleFileNameA (declared inline
 *           to avoid #include <windows.h> which clashes with swanstation types).
 ...
 * (e.g. "swanstation_libretro" or "swanstation_libretro_android"). */
...
static void autoload_logf(const char *fmt, ...)
{
   ...
   if (s_libretro_log_callback_valid && s_libretro_log_callback.log)
      s_libretro_log_callback.log(RETRO_LOG_INFO, "[autoload] %s\n", line);
 ...
 * passed as "<archive>.chd#<inner>" or similar by using the inner name. */
 ...
 * (SwanStation's retro_unserialize reads the buffer directly, so feeding it
 *  the raw RASTATE header would corrupt the load - unwrapping is required.) */
...
static void autoload_try_load_state(void)
{
   ...
   {
      if (s_libretro_log_callback_valid && s_libretro_log_callback.log)
         s_libretro_log_callback.log(RETRO_LOG_WARN,
...
RETRO_API bool retro_load_game(const struct retro_game_info* game)
{
  ...
  /* ---- end autoload: capture ROM path ---- */

  bool result = g_host_interface_storage.retro_load_game(game);

  /* ---- autoload: mark pending on success ---- */
  if (result)
    autoload_state_pending = true;
  /* ---- end autoload: mark pending on success ---- */
